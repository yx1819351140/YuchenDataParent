 ...
 **/
public class Events2EsProcessOperator extends RichMapFunction<JSONObject, JSONObject> {
    ...
    public Events2EsProcessOperator(TaskConfig taskConfig) {
    ...
    public JSONObject map(JSONObject value) throws Exception {
        ...
        String newsId = data.getJSONObject("nlp_res").getString("title_id");
        String pubTime = data.getJSONObject("nlp_res").getString("pub_time");
        newData.put("news_id", newsId);
        newData.put("raw_nlp_result", data.toJSONString()); // 原始nlp结果存成一个json字符串
        newData.put("create_time", pubTime);
        newData.put("update_time", pubTime);
        value.put("isUpdate", true);
        value.put("indexName", generateDynIndexName(pubTime));
