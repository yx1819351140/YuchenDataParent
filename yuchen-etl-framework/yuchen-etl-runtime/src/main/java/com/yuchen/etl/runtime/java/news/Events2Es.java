...
import com.yuchen.etl.runtime.java.news.sink.EventsEsSink;
...
//        String outputTopics = taskConfig.getStringVal("news.output.topics");
        ...
        // 发送Kafka的数据过滤处理
        EventsSinkEsFilter eventsSinkEsFilter = new EventsSinkEsFilter();
        SingleOutputStreamOperator<JSONObject> filterResultNewsStream = kafkaSource.filter(eventsSinkEsFilter).name("过滤掉不需要的事件");

        // 更新标志位 + final标志位生成
        Events2EsProcessOperator events2EsProcessOperator = new Events2EsProcessOperator(taskConfig);
        SingleOutputStreamOperator<JSONObject> finalNewsStream = filterResultNewsStream.map(events2EsProcessOperator).name("统一事件入库");

        // 写出到origin_news_xxxx
        BaseConfig yuchenNewsConfig = taskConfig.getBaseConfig("yuchen_news");
        EventsEsSink eventsEsSink = new EventsEsSink(taskConfig, yuchenNewsConfig);
        finalNewsStream.addSink(eventsEsSink).name("统一事件入库程序");
