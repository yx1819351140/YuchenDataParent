...
import com.yuchen.etl.runtime.java.news.sink.CoEventsEsSink;
...
//        String outputTopics = taskConfig.getStringVal("news.output.topics");
        ...
        // 发送Kafka的数据过滤处理
        CoEventsSinkEsFilter coEventsSinkEsFilter = new CoEventsSinkEsFilter();
        SingleOutputStreamOperator<JSONObject> filterResultNewsStream = kafkaSource.filter(coEventsSinkEsFilter).name("过滤掉不需要的事件");

        // 更新标志位 + final标志位生成
        CoEvents2EsProcessOperator coEvents2EsProcessOperator = new CoEvents2EsProcessOperator(taskConfig);
        SingleOutputStreamOperator<JSONObject> finalNewsStream = filterResultNewsStream.map(coEvents2EsProcessOperator).name("共指事件入库");

        // 写出到origin_news_xxxx
        BaseConfig yuchenNewsConfig = taskConfig.getBaseConfig("yuchen_news");
        CoEventsEsSink coEventsEsSink = new CoEventsEsSink(taskConfig, yuchenNewsConfig);
        finalNewsStream.addSink(coEventsEsSink).name("共指事件入库程序");
